...
static XYPixel XYStack[1000], *Next = &XYStack[0];
...
static int PushPixel(XYPixel p1)
{
...
static int PopPixel(XYPixel *theXYPixel)
{
    if(Next >= XYStack) {
        *theXYPixel = *(--Next);
        return 0 ;
...
********************************************************************************************/
void fill_shape(int _x, int _y, int _FillColour, int _BoundaryColour)
{
    register int     x, y ;
    ...
    while(!IsStackEmpty())                 // keep going until no more items on the stack
    {
        ...
        x = aPoint.x ;
        y = aPoint.y ;
        write_pixel(x, y, FillColour);     // fill the point in the fill colour
        ...
        while((char)(read_pixel(x,y)) != (char)(BoundaryColour))							// if new pixel is not the boundary colour
            write_pixel(x++, y, FillColour);     											// fill it and keep moving right along a horizontal line
        ...
        while((char)(read_pixel(x,y)) != (char)(BoundaryColour))						// if new pixel is not the boundary colour
            write_pixel(x--, y, FillColour);    											// fill it and keep moving left along a horizontal line
        ...
        while(x <= XRight)
        {
            ...
            while(((char)(read_pixel(x,y)) != (char)(BoundaryColour)) && (x <= XRight))
               ++x ;
